...
SIM_UDID := $(shell xcrun simctl list devices available -j | python3 -c "import json,sys; d=json.load(sys.stdin)['devices']; c=sorted([(rt,x) for rt,xs in d.items() if 'iOS' in rt for x in xs if 'iPad' in x['name']]); print(c[-1][1]['udid'] if c else '')")
